...
68. A persistence backend is selected by an explicit, named configuration flag — never inferred from `NODE_ENV`, never silently substituted for another backend on misconfiguration. Business logic depends on the storage interface (`FulfillmentJobStore`), never a concrete backend class. A claim that a given backend's atomicity/restart/rollback property is proven requires that property to have been exercised against a real instance of that backend — an implementation proven only against a different backend, or only by code review, must be reported as unproven, never silently reported as passing (Gate 11).
