...
### DEC-026 — Cactus's public-page resolution is real live infrastructure, not a fallback of last resort; `DATABASE_URL` and several other declared env vars are currently unconsumed and SQLite persistence does not survive Vercel's serverless filesystem

Date: 2026-09-17
Status: LOCKED
Context: Prior product copy described Gate 1A's resolution path as an "SSR fallback... because no Cactus API key was available" — technically accurate but easy to misread as "the real integration failed and something degraded stood in for it." A source-level audit was also requested of every `DATABASE_URL`/`postgres`/`Supabase`/`SQLite` reference in the repository, to determine the actual runtime persistence architecture before any Vercel deployment, rather than assuming `.env.example`'s `DATABASE_URL=postgresql://...` placeholder implies a live Postgres dependency.
Decision:
1. **Cactus resolution language is corrected everywhere it was confusing**, not just softened. `resolveViaSsrFallback` (`packages/cactus/src/ssr-fallback.ts`) performs a real, live `fetch()` against the actual Cactus proposal page and parses the same `__NEXT_DATA__` hydration payload the production Cactus app itself renders from — genuine current infrastructure, not a mock, not a simulation, not fixture data standing in for a live call. The corrected language (docs, README, the landing FAQ, `/app/new`, `claims-registry.ts`) states this precisely: two resolution paths reach the same real Cactus data (official GraphQL API when `CACTUS_API_KEY` is set; the live public-page path otherwise, the current default), and neither is described as degraded relative to the other except in the specific, narrow sense that the API path is authenticated and this environment has no credential for it.
2. **`DATABASE_URL` is confirmed, by source search rather than assumption, to be completely unconsumed.** It exists as one `z.string().optional()` field in `packages/config/src/env.ts`'s `EnvSchema`, and that schema's only caller anywhere in the repository is its own test file — `parseEnv()` is never invoked by `apps/web`, `apps/cli`, or `scripts/`. There is no Postgres/Drizzle client anywhere in this codebase. `.env.example` now labels it explicitly as reserved for `packages/db/README.md`'s already-documented future Postgres/Drizzle target, not as a currently-required value.
3. **Real runtime persistence is a hardcoded local SQLite file**, `apps/web/src/lib/job-store.ts`'s `join(process.cwd(), ".data", "marked.sqlite")` via `node:sqlite` — no env var controls its location at all. Vercel's Node serverless functions run against a read-only bundle with only `/tmp` writable and not guaranteed to persist across invocations; this means ARM/APPROVE/DISARM and the recovery sandbox cannot be relied on to persist between two requests to a Vercel deployment of this app today. This was not previously documented anywhere as a deployment-blocking caveat and is now stated plainly in `VERCEL_ENVIRONMENT.md`.
4. **Several other declared env vars are equally inert today**, confirmed the same way (source search, not assumption): `KEEPERHUB_API_KEY`/`KEEPERHUB_BASE_URL` are read only by local one-off scripts, never by the deployed app; `ENABLE_MAINNET_WRITE` is declared and unit-tested but the one real caller that matters (`scripts/gate5-keeperhub-execute.ts`) hardcodes `false` as a literal rather than reading it from `process.env`; `ENABLE_PUBLIC_RECEIPTS` and `NEXT_PUBLIC_APP_ENV` are declared/defaulted/tested but never read to gate or render anything. All are listed in `VERCEL_ENVIRONMENT.md`'s "NOT REQUIRED" section rather than left ambiguous.
Reason: "Governance is not finished when it passes; it is finished when it is Marked" applies to this project's own documentation as much as to a DAO's proposal — a claim about what an environment variable does is either verified against real source usage or it is not made. Silently assuming `DATABASE_URL` implied a Supabase/Postgres dependency (it does not) would have been exactly the kind of unverified claim this project's own discipline forbids elsewhere.
Evidence: `VERCEL_ENVIRONMENT.md` (the full audit table, source-verified); `.env.example` and `apps/web/.env.example` (updated labeling); `packages/db/README.md` (pre-existing statement of the Postgres/Drizzle future target, unchanged); the corrected Cactus language in `README.md`, `apps/web/src/lib/docs-content.ts` (`cactus-integration` doc entry), `apps/web/src/lib/claims-registry.ts` (Gate 1A card), `apps/web/src/app/(site)/page.tsx` (FAQ), and `apps/web/src/app/app/new/page.tsx`.
Consequences: Deploying this app to Vercel today should not be read as claiming durable fulfillment-job persistence in production — that requires the `DATABASE_URL`/Postgres migration `packages/db/README.md` already anticipated, which has not happened yet. Any future gate that wires up that migration must update `VERCEL_ENVIRONMENT.md`'s "NOT REQUIRED" row for `DATABASE_URL` back to "required," not leave both documents disagreeing.

### DEC-027 — Production persistence is a real PostgreSQL adapter behind the existing `FulfillmentJobStore` abstraction, selected by an explicit driver flag that fails closed, proven against a real database engine wherever one was reachable, and honestly reported as unproven against hosted Postgres where none was

Date: 2026-09-17
Status: LOCKED (persistence architecture); the hosted-Postgres-exercised claim specifically remains BLOCKED pending user-supplied `DATABASE_URL` — see `evidence/production-persistence/gate11-result.md`
Context: DEC-026 established that DEC-021's local-file SQLite store cannot serve as durable persistence on Vercel's serverless filesystem. Gate 11's mandate was to close that gap with real PostgreSQL persistence, without redesigning Marked's authority model, without a silent fallback to SQLite in production, and without claiming a hosted database was exercised unless it genuinely was.
Decision:
1. **`FulfillmentJobStore` gained new members, additively.** `tryClaimExecution`/`getExecutionClaim`/`close` — previously `SqliteFulfillmentJobStore`-only, which meant `apps/web/src/lib/fulfillment-actions.ts` and `job-store.ts` had to depend on that concrete class rather than the interface — are now part of the interface itself. `saveJobAndEvents` (one atomic transaction for a job write + its event append, closing a real durability gap where the two were previously two separate, non-atomic calls) and `saveWithCas` (a job-level compare-and-set, on top of Gate 7's existing execution-claim CAS) are new. The original five methods' signatures are unchanged.
2. **`PostgresFulfillmentJobStore` uses `postgres` (porsager/postgres) directly, not an ORM.** Verified live during this gate (Vercel's own current documentation) that first-party "Vercel Postgres" was discontinued in December 2024 and existing databases were migrated to Neon, with new projects routed through the Vercel Marketplace. Given that, and given five methods over three tables, raw parameterized SQL — mirroring the existing `SqliteFulfillmentJobStore`'s own style — was judged sufficient; Drizzle (the originally-`packages/db/README.md`-documented target) was evaluated and not chosen, to avoid an ORM dependency the actual surface area doesn't need. Neither Supabase nor any Vercel- or Neon-specific driver was introduced — `postgres` works against any standard Postgres.
3. **Driver selection is explicit and fails closed.** `apps/web/src/lib/job-store.ts`'s `buildStore()` reads `MARKED_STORAGE_DRIVER` (`sqlite`, the default, or `postgres`) — never `NODE_ENV`-based inference. `postgres` selected without `DATABASE_URL` throws `PersistenceConfigurationError` immediately; it never silently constructs a SQLite store instead. This is the same no-silent-fallback discipline DEC-013 (Cactus) and DEC-024 (agent provider) already established, applied here to the storage layer.
4. **Every property provable without a live Postgres connection was proven for real, against a real database engine (SQLite via `node:sqlite`, not a mock).** The new `saveWithCas`/`saveJobAndEvents` methods, a genuine two-concurrent-caller race for both the execution-claim CAS and the new job-level CAS, and a genuine forced mid-transaction rollback (a `BigInt` field that makes `JSON.stringify` throw *inside* an open transaction, after the first statement already ran) all pass — see `evidence/production-persistence/{atomicity,rollback-proof}.md`. A shared contract test suite (`packages/db/src/fulfillment-job-store.contract.ts`) runs the identical assertion battery against both `SqliteFulfillmentJobStore` and `PostgresFulfillmentJobStore` — the SQLite side passes for real; the Postgres side is structured identically but is explicitly `it.skip`'d with its exact reason printed, because no local, containerized, or hosted Postgres was reachable in this environment (no Docker, no local `psql`; this repository's own `.env.local` `DATABASE_URL` is still the unedited `.env.example` placeholder, confirmed by reading the file). A skipped test is never reported as a passing proof.
5. **A real build-time side effect was found and fixed, not merely documented.** `pnpm build` was empirically shown (clean `.data/` removed, isolated `pnpm --filter @marked/web build`, `.data/marked.sqlite` reappeared) to execute `apps/web/src/app/app/page.tsx`'s `getAppStore()` call during Next's build-time static/dynamic classification pass, even though that route is ultimately dynamic. Fixed with `export const dynamic = "force-dynamic"` on the three job-store-backed pages; re-tested and confirmed fixed. Left unfixed, this would have made `next build` itself open a live Postgres connection once `MARKED_STORAGE_DRIVER=postgres` is configured — a real deployment risk this gate would otherwise have shipped silently.
Reason: Structural equivalence between two implementations of one interface, proven via one shared test suite rather than two independently-written ones, is the correct proof shape here specifically because the point is interchangeability — the same discipline as Gate 8's independent-verifier pattern, adapted to "same behavior across two backends" rather than "same statistic computed twice." Reporting the hosted-Postgres gap as BLOCKED rather than a downgraded PASS matches the explicit instruction governing this gate and the project's standing rule (DEC-010, DEC-024) that a live capability is only claimed once actually exercised.
Evidence: `evidence/production-persistence/` (nine files — audit, architecture, schema, atomicity, restart-proof, rollback-proof, environment-audit, security-scan, gate11-result), `evidence/production-persistence/packages-db-test-output.txt`, `packages/db/migrations/0001_init.sql`, `VERCEL_ENVIRONMENT.md` (updated).
Consequences: The moment a real `DATABASE_URL` is supplied and migrated, `pnpm --filter @marked/db test` will automatically stop skipping the Postgres contract suite and either prove or disprove the implementation for real — no code change is needed to "turn on" that proof, only a real database. Until then, any product copy or claims-registry entry describing Postgres persistence must say "implemented, proven identical to SQLite via a shared contract suite, not yet exercised against a live Postgres server" — never "production-tested" or equivalent unqualified language.
