...
STATUS: **PASS** (see `evidence/production-persistence/gate11-result.md`)
...
PASS CONDITION: A production-compatible PostgreSQL persistence adapter exists, satisfies the exact same `FulfillmentJobStore` interface as `SqliteFulfillmentJobStore`, never silently falls back to SQLite when misconfigured, and its atomic/restart/rollback properties are proven against a real database engine — plus the full existing regression suite passes with zero new blockchain writes. **Met, including the hosted-Postgres-exercised claim**: a real Neon `DATABASE_URL` was supplied, migrated for real (`pnpm db:migrate`), and the full mandatory test battery — shared contract suite, concurrent-worker races, forced-failure transaction rollback, restart/reconnect, JSONB serialization round-trip — was run against it for real, all passing (`evidence/production-persistence/hosted-production-proof.md`, raw output in `evidence/production-persistence/hosted-postgres-test-output.txt`). `PostgresFulfillmentJobStore` (`packages/db/src/postgres-fulfillment-job-store.ts`) is proven identical in behavior to `SqliteFulfillmentJobStore` via the same shared contract test suite (`packages/db/src/fulfillment-job-store.contract.ts`) run against both backends. Two genuine test-infrastructure gaps (cross-file table sharing against one persistent database; no per-test isolation for fixed contract ids) and one genuine Neon cold-start connect-timeout issue were found by this real exercise and fixed — not worked around — per the standing "do not weaken the test to get a PASS" instruction; full account in `hosted-production-proof.md`.

A real bug was found and fixed as part of meeting this gate's "Vercel build passes" requirement: `pnpm build` was empirically shown to open a real database connection as a side effect of Next.js's build-time static/dynamic classification pass (traced to `apps/web/src/app/app/page.tsx` calling `getAppStore()` unconditionally at render time). Fixed with `export const dynamic = "force-dynamic"` on the three job-store-backed pages; re-tested and confirmed a clean build no longer touches persistence at all — re-confirmed again after the hosted-DB work. See `evidence/production-persistence/architecture.md`.

KILL / BLOCK CONDITION: Silent fallback from Postgres to SQLite on misconfiguration, a fabricated or mocked hosted-database proof, business logic depending on a concrete store class instead of `FulfillmentJobStore`, a destructive migration, or any new blockchain write. Not triggered — `buildStore()` (`apps/web/src/lib/job-store.ts`) throws `PersistenceConfigurationError` rather than falling back; `fulfillment-actions.ts`/`job-store.ts` are retyped to the interface; `packages/db/migrations/0001_init.sql` contains only `CREATE TABLE IF NOT EXISTS`/`CREATE INDEX IF NOT EXISTS` statements, never a `DROP`; zero `sendTransaction`/signer/private-key usage anywhere in this gate's changes, including the hosted-DB follow-up work. The hosted-persistence proof was captured only once genuinely earned, per the standing instruction not to fabricate it.

EVIDENCE REQUIRED: `evidence/production-persistence/{current-storage-audit,architecture,schema,atomicity,restart-proof,rollback-proof,environment-audit,security-scan,gate11-result,hosted-production-proof}.md`, `evidence/production-persistence/{packages-db-test-output,hosted-postgres-test-output}.txt`, updated `VERCEL_ENVIRONMENT.md`, `packages/db/migrations/0001_init.sql`, `packages/db/vitest.config.ts`, `packages/db/src/postgres-fulfillment-job-store.hosted.test.ts`, `pnpm db:migrate`. Present — all listed files exist and are accurate as of this entry.
...
| 11 — Production persistence + Vercel deployment readiness | **PASS** (hosted Neon Postgres migrated and exercised by the full mandatory test battery, for real) |
