...
See `PRD.md` (Marked PRD v1.2 LOCK) for the full product specification. See `BUILD_CONTRACT.md` for the laws implementation may never violate.
...
packages/config       Strict environment parsing (fail-safe: ENABLE_MAINNET_WRITE defaults false)
...
packages/db           SqliteFulfillmentJobStore — real file-backed persistence (Gate 7)
...
- Gate 1A — Cactus seam: **PASS** (via the documented SSR fallback; official authenticated GraphQL API is `BLOCKED_CACTUS_CREDENTIAL`)
...
- Gate 11+ onward: not yet completed
...
- **Live Cactus proposal resolution** — real mainnet proposals (Compound #220, Uniswap #20) resolved to organization/chain/Governor/onchain proposal ID through the current, documented SSR fallback seam, independently cross-checked on-chain. See `evidence/cactus/`.
...
- **Durable persistence and fail-closed recovery** — a real, file-backed `node:sqlite` database survives an actual process restart (proven against the live running app, not only in-process), duplicate-worker protection is a genuine SQL compare-and-set, and every ambiguous-execution recovery outcome is typed so that resubmission is structurally unrepresentable, never merely disallowed by a runtime check. See `evidence/recovery-hardening/` and `DEC-021`.
...
- Persistence (`node:sqlite`) is single-file/single-process — not distributed/multi-node production infrastructure.
