...
|---|---|---|
| Unauthenticated visitor | Every static page; any job's full history by id (`GET /api/fulfillment/state`, intentionally public); the 4 agent Server Actions (rate-limited); a self-issued demo session (see Authentication below) | Read any job by id; after a trivial "login," mutate any job's demo state; **cannot reach any real blockchain effect** — see Write-path boundary |
| Authenticated demo operator (self-issued) | ARM/APPROVE/DISARM on any job | Same technical reach as above — there are no privilege tiers in the current demo model |
| Compromised/malicious LLM output | Feeds the deterministic plan validator and rendered explanation text | Cannot mutate job state (every agent action is read-only, test-proven); cannot smuggle an authority-shaped field past the validator (the schema has no such field) |
| Concurrent callers/serverless workers | Any state-changing operation | ARM/APPROVE/DISARM and execution-claim races are both now CAS-protected (Gate 12/Gate 7) |

## Write-path boundary (the single most important fact for interpreting everything below)

**The deployed web application has no code path capable of a real blockchain write.** Repo-wide grep for `sendTransaction`/`writeContract`/`signTransaction`/`privateKeyToAccount`/`eth_sendRawTransaction`/`walletClient` finds exactly one match anywhere in the repository, an offline proof script never imported by `apps/web`. `packages/keeperhub`'s `executeContractCall` — the one function capable of a real KeeperHub-mediated execution — is imported only by its own tests and two offline proof scripts, never by `apps/web/src`. Every real on-chain write this project has performed (Gate 2's deployment, Gate 5's KeeperHub-mediated Sepolia execution) was run by a developer invoking a script directly, never through the deployed app.

**Consequence**: several findings below that would be S0/S1 in a system that could execute are correctly S1/S2 here, because they are bounded to internal state/audit-trail impact, not fund-loss or unauthorized execution. This bound disappears the moment execution is wired into the live app (see `docs/ARCHITECTURE.md` §32) — re-read this document before doing that.
