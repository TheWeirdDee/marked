...
**This is finding F-03** (see `findings.md` for full detail, severity reasoning, and why it is not being redesigned this gate). In short: severity is S1 by the letter of the model, but the real-world impact is bounded to "any visitor can mutate any demo job's internal status and pollute the actor-name audit trail" — at the time of this audit, it could not reach a real blockchain effect, because the live app had no execution write-path at all (F-06). **Updated 2026-09-18: F-06 is now superseded — a real execution path exists, bounded instead by a single default-off flag, `MARKED_ENABLE_KEEPERHUB_EXECUTION`. This finding's impact is bounded only as long as that flag stays off. See `findings.md` F-03's correction note and `DECISIONS.md` DEC-030.**
