...
**Conclusion (see finding F-06 in `findings.md`): the deployed web application has no code path capable of a real blockchain write.** Every actual on-chain write this project has ever performed (Gate 2's Governor deployment, Gate 5's KeeperHub-mediated Sepolia execution) was performed by an offline, developer-run script, never by the live app. This is the single most important piece of context for interpreting the severity of several other findings in this audit (F-01, F-03) — a bug in the live app's mutation path cannot today cause an unauthorized on-chain transaction, because the live app cannot cause an on-chain transaction of any kind.
